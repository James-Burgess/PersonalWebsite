import { allCvs } from "../../cv/cv-index.js";
	...
	async fetch(request, env, ctx) {
		...
		if (url.pathname === "/contact") {
			if (request.method === "GET") {
				return new Response(
					renderContactForm(false, false, env.TURNSTILE_SITE_KEY),
					{
						headers: { "Content-Type": "text/html" },
					},
				);
			...
			if (request.method === "POST") {
				try {
					...
					if (!verifyData.success) {
						return new Response(
							renderContactForm(false, "captcha", env.TURNSTILE_SITE_KEY),
							{
								headers: { "Content-Type": "text/html" },
							},
						);
					...
					return new Response(
						renderContactForm(true, false, env.TURNSTILE_SITE_KEY),
						{
							headers: { "Content-Type": "text/html" },
						},
					);
					...
					return new Response(
						renderContactForm(false, "send", env.TURNSTILE_SITE_KEY),
						{
							headers: { "Content-Type": "text/html" },
						},
					);
